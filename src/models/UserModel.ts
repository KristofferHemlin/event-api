...
export default class UserModel {
  getAllUsers(additionalRelations: string[] = []): Promise<User[]> {
    return getRepository(User)
      .find({ relations: additionalRelations, order: { id: "ASC" } })
      .catch(error => {
  ...
  getUsersOn(
    ...
    try {
      return this.fetchParticipantBuilder(
        relationType,
        relationId,
        additionalRelations,
        ""
        ...
        .catch(error => {
          console.error(
            `Error while fetching users for ${relationType}:`,
            error
          );
          return Promise.reject(error);
        });
  ...
  getUsersCount(table: string, id: number, searchValue): Promise<number> {
    try {
      return this.fetchParticipantBuilder(table, id, [], searchValue)
        .getCount()
        .catch(error => {
  ...
  getActivityUpdateLogs(
    userId: number,
    limit: number
  ): Promise<ActivityUpdateLog[]> {
  ...
  saveUsers(users: User[]): Promise<number> {
    ...
    return createQueryBuilder(User)
      ...
      .execute()
      .then(insertResult => {
        const numInsterted = insertResult.identifiers.reduce(
          (numRecords, record) => {
            if (record) {
              numRecords++;
            }
  ...
  private getUserBy(
    ...
    try {
      ...
      return queryBuilder
        .where(`User.${type} = :identifier`, { identifier: typeIdentifier })
        .getOne()
        .catch(error => {
  ...
  private getUsersBy(
    ...
    try {
      ...
      return queryBuilder
        .where(`User.${type} IN (:...identifiers)`, {
  ...
  private fetchParticipantBuilder(
    table: string,
    id: number,
    additionalRelations: string[],
    searchValue: string
    ...
    queryBuilder.innerJoin("User." + table, table, table + ".id=:id", {
      id: id
    });
    additionalRelations.forEach(relation => {
      queryBuilder.leftJoinAndSelect("User." + relation, relation);
    ...
    if (searchValue) {
      ...
      queryBuilder.where(
        `LOWER(User.firstName) LIKE :searchParam 
                    OR LOWER(User.lastName) LIKE :searchParam 
                    OR LOWER(User.companyDepartment) LIKE :searchParam`,
